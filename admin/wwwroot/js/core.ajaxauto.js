...
$.fn.ajaxJson = function(json) {
    ...
    if(json.tipContext && json.msg) {
        var tip = eval(json.tipContext);
        tip.after("<span class='tip-error' id='field-tip-" + tip.attr('name') + "'><i class='fa fa-times'></i>"+ json.msg +"</span>");
        tip.focus();
    } else if (json.status == 'ok' || json.status == 'error') {
        if (json.msg) {
            if (json.redirect) {
//                $.alert(json.msg, redirect);
                $.status(json.msg);
                //return true;
