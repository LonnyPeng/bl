...
if (window.jQuery)(function ($) {
			...
			.each(function () {
				...
				MultiFile.addToList = function (slave, slave_count, files) {
					...
					//$.each(files, function (i, file) {
						...
						var
							r = $('<div class="MultiFile-label-wrap"></div>'),
	...
	*/
	$.fn.MultiFile.options = { //$.extend($.fn.MultiFile, { options: {
		...
		// previews
		preview: false,
		previewCss: 'height:100px; width:100px; display:block;',
		...
		STRING: {
			...
			selected: '选择文件: $file',
			duplicate: '此文件已被选中:\n$file',
			toomuch: '所选择的文件超过允许的最大小 ($size)',
			toomany: '选择的文件太多 (最多: $max)',
			toobig: '$file 太大 (最大 $size)'
		},
